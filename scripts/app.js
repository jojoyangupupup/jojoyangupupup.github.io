...
function renderMemoryExperience(documentEntry) {
  ...
  return `<article class="memory-experience memory-archive" aria-labelledby="memory-title-${documentEntry.id}">
    <section class="memory-section memory-hero"><div class="memory-hero-copy"><h2 class="memory-major-title" id="memory-title-${documentEntry.id}">${memory.title}</h2><p class="memory-tagline">${memory.heroTagline}</p><p class="memory-hero-copy-text">${memory.productIntro}</p></div><div class="memory-interface" aria-label="AI 知识加工台界面示意"><div class="memory-interface-top"><span></span><span></span><span></span></div><div class="memory-interface-body"><i></i><i></i><i></i><b></b></div></div></section>
    <section class="memory-section memory-role"><div><h3>02 我的成果</h3><p>围绕重点功能迭代开展运营，把真实工作困境转化为用户愿意尝试的使用场景。</p><div class="memory-role-actions"><div><strong>场景提炼</strong><span>从真实工作困境中找到内容切口</span></div><div><strong>内容触达</strong><span>用上新和场景内容解释功能价值</span></div><div><strong>体验转化</strong><span>推动用户从知道产品到实际使用</span></div><div><strong>数据复盘</strong><span>根据行为数据调整内容重点</span></div></div></div></section>
    ...
    <section class="memory-section memory-data"><header><p class="memory-major-title">03 数据效果</p><h3>从创建入口，到实际使用，再到持续提问</h3></header><div class="memory-kpis">${memory.kpis.map((item) => `<div class="memory-kpi"><strong>${item.value}</strong><span>${item.label}</span></div>`).join("")}</div><div class="memory-monthly"><h4>月度增长对比</h4>${memory.monthly.map((item) => `<div class="memory-month-row"><strong>${item.label}</strong><span class="memory-month-bar"><i style="--old:${item.oldPercent}%;--new:${item.newPercent}%"></i></span><em>${item.old} → ${item.new} · ${item.delta}</em></div>`).join("")}</div><div class="memory-path"><h4>用户行为路径</h4><div>${memory.path.map((item, index) => `<span><b>${item.count}</b><small>${item.label}</small></span>${index < memory.path.length - 1 ? `<i>${index === 0 ? "77.1% 创建用户进入实际使用" : "→"}</i>` : ""}`).join("")}</div></div><p class="memory-data-note">数据区间：2026.06.01—2026.08.04，约 10 周</p><p class="memory-insight">${memory.insight}</p></section>
