...
function modelPrdCardMarkup(prd, index) {
  return `<button type="button" class="model-prd-card${index === 0 ? " is-active" : ""}" data-model-prd-index="${index}" data-model-prd-phase="${modelPrdPhase(prd)}"><span>${prd.number}</span><strong>${prd.title}</strong><small>${prd.time} · ${prd.theme}</small><em>${prd.status || "已完成 / 已上线"}</em></button>`;
}
...
function renderModelExperience(documentEntry) {
  ...
  const phaseFilters = ["全部", "前置", "1.0", "2.0", "3.0", "4.0"];
