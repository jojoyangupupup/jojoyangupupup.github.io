...
function renderMemoryExperience(documentEntry) {
  ...
  return `<article class="memory-experience memory-archive" aria-labelledby="memory-title-${documentEntry.id}">
    ...
    <section class="memory-section memory-content"><header><p class="memory-major-title">04 精选内容</p><h3>用场景内容把功能讲清楚</h3></header><div class="memory-content-browser"><nav aria-label="精选内容目录">${memory.contentArchive.map((item, index) => `<button type="button" class="memory-content-tab${index === 0 ? " is-active" : ""}" data-content-index="${index}"><span>${item.number}</span><strong>${item.type}</strong><small>${item.title}</small></button>`).join("")}</nav><div class="memory-content-detail" aria-live="polite"></div></div></section>
  </article>`;
}
