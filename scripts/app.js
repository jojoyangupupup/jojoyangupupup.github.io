...
function renderMemoryExperience(documentEntry) {
  ...
  return `<article class="memory-experience memory-archive" aria-labelledby="memory-title-${documentEntry.id}">
    <section class="memory-section memory-hero"><div class="memory-hero-copy"><p class="memory-index">01 产品介绍</p><h2 class="memory-major-title" id="memory-title-${documentEntry.id}">${memory.title}</h2><p class="memory-tagline">把企业里散落的知识、材料和文件，转成能直接使用的工作成果。</p><p class="memory-hero-copy-text">面向办公场景，把周报、会议纪要、知识库文档、本地文件等内容，生成 PPT、信息图、总结、问答等可继续使用的结果。</p></div><div class="memory-interface" aria-label="AI 知识加工台界面示意"><div class="memory-interface-top"><span></span><span></span><span></span></div><div class="memory-interface-body"><i></i><i></i><i></i><b></b></div></div></section>
    <section class="memory-section memory-role"><div><p class="memory-index">02 我的成果</p><p>围绕重点功能参与场景提炼、运营内容策划、使用引导和数据复盘。</p><div class="memory-role-actions"><div><strong>场景提炼</strong><span>从真实工作困境里找用户会共鸣的切口</span></div><div><strong>内容策划</strong><span>围绕功能上新、场景教育和使用教程做内容</span></div><div><strong>体验转化</strong><span>让用户从知道产品变成创建、使用、提问</span></div><div><strong>数据复盘</strong><span>根据行为数据调整运营重点</span></div></div></div></section>
    <section class="memory-section memory-features"><header><p class="memory-major-title">02 我的成果 · 功能运营</p><h3>把功能迭代，变成用户能理解、愿意尝试的工作场景</h3></header><div class="memory-feature-list">${memory.features.map((feature) => `<article class="memory-feature"><div class="memory-feature-title"><span>${feature.number}</span><h4>${feature.title}</h4></div><div><small>功能迭代</small><p>${feature.iteration}</p></div><div><small>运营内容</small><p>${feature.operation}</p><blockquote>“${feature.expression}”</blockquote></div></article>`).join("")}</div></section>
    <section class="memory-section memory-data"><header><p class="memory-major-title">03 数据效果</p><h3>从创建入口，到实际使用，再到持续提问</h3></header><div class="memory-kpis" role="tablist" aria-label="数据指标">${memory.monthly.map((item, index) => `<button type="button" class="memory-kpi" role="tab" aria-selected="${index === 0}" data-memory-kpi-index="${index}"><strong>${item.new}</strong><span>${item.label}</span></button>`).join("")}</div><div class="memory-metric-detail" aria-live="polite"></div><div class="memory-path"><h4>用户行为路径</h4><div>${memory.path.map((item, index) => `<span><b>${item.count}</b><small>${item.label}</small></span>${index < memory.path.length - 1 ? `<i>${index === 0 ? "创建 → 使用" : "→"}</i>` : ""}`).join("")}</div></div><p class="memory-data-note">数据区间：2026.06.01—2026.08.04，约 10 周</p><p class="memory-insight">${memory.insight}</p></section>
    <section class="memory-section memory-content"><header><p class="memory-index">运营内容档案</p><h3>真实运营内容，直接说明每次迭代如何被讲清楚</h3></header><div class="memory-content-browser"><nav aria-label="内容档案目录">${memory.contentArchive.map((item, index) => `<button type="button" class="memory-content-tab${index === 0 ? " is-active" : ""}" data-content-index="${index}"><span>${item.number}</span><strong>${item.title}</strong></button>`).join("")}</nav><div class="memory-content-detail" aria-live="polite"></div></div></section>
  </article>`;
}
